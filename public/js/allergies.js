//updated search button onClick for Autocomplete and some validation for the form
 ...
 $('#searchButton').on('click', (e) => {
     ...
     if (input != '' && ingredientSearch == null) {
         ...
         var allergen = $('#allergen-name').val().trim();
         if ((allergen == null)) {
          alert('Please pick an allergy!!');
          return false;
          }else {
          getRecipes(keyword, allergen);
         ...
         var allergen = $('#allergen-name').val();
         if ((allergen == null)) {
          alert('Please pick an allergy!!');
          return false;
          }else {
 ...
 var getRecipes = function(keyword, allergen) {
         axios.get("https://api.yummly.com/v1/api/recipes?_app_id=67b9fed7&_app_key=3694f49499f4bce2a7594b8b1ac27168&q=" + keyword + "&allowedAllergy[]=" + allergen + "&requirePictures=true &maxResult=10&start=10")
             .then((result) => {
                 console.log(result);
                 let recipes = '';
