 ...
 $('#searchButton').on('click', (e) => {
     ...
     if (input != '' && ingredientSearch == null) {
         ...
         var diet = $('#diet input:checked').val();
         var allergen = $('#allergies input:checked').val();
         var cuisine = $('#cuisine input:checked').val();

         getRecipes(keyword, allergen, cuisine, diet);
     }
 });
 // loading page--- gets the image, name and recipe ID --
 var getRecipes = function(keyword, allergen, cuisine, diet) {
         axios.get("https://api.yummly.com/v1/api/recipes?_app_id=67b9fed7&_app_key=3694f49499f4bce2a7594b8b1ac27168&q=" + keyword + "&allowedDiet[]=" + diet + "&allowedAllergy[]=" + allergen + "&allowedCuisine[]=" + cuisine + "&requirePictures=true &maxResult=10&start=10")
             .then((result) => {
                 console.log(result);
                 let recipes = '';
                    ...
                    <div class="col-md-8">
                      <h2>Nutrition Facts </h2>
                        <ul class="list-group">
                          ...
                          <a href="/allergies" class="btn btn-default">Go Back To Search</a>
